findM4Upper:=function(N1, N2)
    ...
    local pathname, filename, outfile, i, j, k, p, q, r, s, numsg, G, H, ms, numms, count, intSubs, nsolv,
                    ...
                    if flag=0 then   # let's just limit ourselves to one M4 per group
                        ...
                        for q in [(p+1)..(numms-2)] do
                            if flag=0 then   # limit to one M4 per group
                                ...
                                # If H is core-free, then G acts faithfully on G/H, so
                                # embeds into the symmetric group on [G:H] letters.  So,
                                # first check necessary condition for core-free:
                                
                                # check |G| divides [G:H]!
                                if RemInt(Factorial(indx),i)=0 then   # recall i=|G|
                                ...
                                if IsNormal(G,H) and H<>e then
                                    ...
                                    if Size(intSubs.subgroups)=4 then
                                        ...
                                        od;  # r
                                    fi; # normal/core-free check
                                    fi; # |G| divides [G:H]! check
                                fi;
                            fi; # if flag=0
                        od;   # q
                    fi;   # if flag=0
