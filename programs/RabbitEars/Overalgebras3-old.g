...
Overalgebra3:=function(args)
    ...
    local i, j, k, kk, z, x, K, aa, a, bb, b, bi, bj, blk, Bmp, B0, B1, BB, p0, p, pi, r, q, g, gp0, gens, Gens, numgens, N, n, 
          nargin, G, pathname, filename, outfile, opcount, pos, VERBOSE, sizeA, algebraname, opname, M, Q, foundx,
          uacalcpath, uacalcfile, STORE_GSET, numalgebras, yetfound, f1, f2;
    ...
    ### TOTALS:  2*Q*K+K potatos ==> (N-1)*(2*Q*K+K) + 1 points
    sizeA:= (N-1)*(2*Q*K+K) +1;
    if VERBOSE then
    ...
    ### Construct p0, p[K], p[2*K], ..., p[2*Q*K] ###
    ...
    for x in [1..sizeA] do
        ...
        if x in B0 then
            ...
            for q in [1..Q] do
                if x in BB[2*q*K] then
                    foundx:=true;
                    p0[x]:=Position(BB[2*q*K], x);
                    for j in [1..Q] do
                        p[2*j*K][x]:=BB[2*j*K][Position(BB[2*q*K], x)];
                        p[(2*j-1)*K][x]:=BB[(2*j-1)*K][b[K-1]];
                    od;
                    break;
                elif x in BB[(2*q-1)*K] then
                    foundx:=true;
                    p0[x]:=a[1];
                    for j in [1..Q] do
                        p[2*j*K][x]:=BB[2*j*K][a[1]];
                        p[(2*j-1)*K][x]:=BB[(2*j-1)*K][Position(BB[(2*q-1)*K], x)];
                    od;
                    break;
                fi;
            od;
                ...
                if x in B0 then
                    ...
                    if not foundx then
                        if x in BB[q*K+k] then 
                            ...
                            for j in [q*K+k+1..2*Q*K+K-1] do  # check if x is in a BB[j] above BB[q*K+k]
                                if x in BB[j] then
                                    foundx:=true;
