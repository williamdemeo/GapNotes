...
#            Print("Output directory: ", pathname, " (plus Gsets\\ or Groups\\)\n");
        else
        ...
        pathname := "/home/williamdemeo/Dropbox/ClosureCore/UACalcFiles/"; # <<< ATTENTION! You should edit this variable.
        if IsDirectoryPath(pathname) then
#            Print("Output directory: ", pathname, "(plus Gsets/ or Groups/) \n");
        else
            Print("Warning: output directory: ", pathname, " is unavailable... \n");
...
gset2uacalc:=function(args)
    ...
    if ARCH_IS_WINDOWS() then
        pathname := Concatenation(pathname, "Gsets\\");
    else
        pathname := Concatenation(pathname, "Gsets/");
    fi;
...
group2uacalc:=function(args)
    ...
    local nargin, G, algebraname, pathname, filename, outfile, N, g, f, h, x, elements, pos, i, j, MAXDISP;
    G:=args[1]; 
    MAXDISP:=16;
    nargin := Length(args);
    if nargin < 2 then 
    ...
    pathname := getPathName();
   if ARCH_IS_WINDOWS() then
        pathname := Concatenation(pathname, "Groups\\");
    else
        pathname := Concatenation(pathname, "Groups/");
    fi;
    filename := Concatenation(pathname, algebraname, ".ua");
...
groups2uacalc:=function(N)
    ...
    #                             as an algebra suitable for loading into the ua calculator.
    #
    local nargin, G, algebraname, pathname, filename, outfile, g, f, h, x, elements, pos, i, j, k, MAXDISP;
    MAXDISP:=16;
    ...
    filename := Concatenation(pathname, "GroupsOfOrder", String(N), ".ua");
    outfile := OutputTextFile(filename, false);
    SetPrintFormattingStatus(outfile, false);   # This prevents automatic indentation and line breaks.
