...
OveralgebraPC:=function(args)
    ...
    # This is a generalized version of the construction described in the section 
    # "Overalgebras I" of version 2.01 of the "Expansions of finite algebras" paper.
    # Here \beta is a principal congruence, say, \beta = Cg(x,y), and we have K 
    # faces strung together at points 
    #
    #           B_0 \cap B_1 = {x} = {x^1}, 
    #           B_1 \cap B_2 = {y^1} = {y^2}, 
    #           B_2 \cap B_3 = {x^2} = {x^3},
...
Overalgebra3:=function(args)
    # Overalgebra3:=function([G, [[a1,b1],[a2,b2],...], q, XO, algebraname])
    # This is the construction described in the section 
    # "Overalgebras III" of version 2.03 of the "Expansions of finite algebras" paper.
    # Here \beta = Cg((a_1,b_1), (a_2,b_2),...,(a_K,b_K)), and we have faces
    # strung together as follows:  
    #
    # B_0 \cap B_1 = {a_1} = {a_1^1}
    # B_1 \cap B_2 ={b^1_1}={a_2^2},
    # B_2\cap B_3 ={b^2_2}={a_3^3},
    # B_{K-2}\cap B_{K-1} = {b_{K-2}^{K-2}}={a_{K-1}^{K-1}},
    # B_{K-1}\cap B_K\cap B_{K+1} ={b^{K-1}_{K-1}}={b^{K}_{K-1}}={b^{K+1}_{K-1}},
    # B_{K+1}\cap B_{K+2}={a^{K+1}_{K-1}} ={b^{K+2}_{K-2}},
    # B_{K+2}\cap B_{K+3}={a^{K+2}_{K-2}} ={b^{K+3}_{K-3}},
    # B_{2K-2}\cap B_{2K-1} = {a_2^{2K-2}}={b_1^{2K-1}},
    # B_{2K-1}\cap B_{2K}\cap B_{2K+1}={a^{2K-1}_1}={a^{2K}_1}={a^{2K+1}_1},
    # B_{2K+1}\cap B_{2K+2}={b^{2K+1}_1} ={b^{2K+2}_2},
    # B_{2K+2}\cap B_{2K+3}={b^{2K+2}_2} ={b^{2K+3}_3},
    # B_{(2q+1)K-2}\cap B_{(2q+1)K-1} = {b_{(2q+1)K-2}^{K-2}}={a_{(2q+1)K-1}^{K-1}},
    # B_{(2q+1)K-1}\cap B_{(2q+1)K}={b^{(2q+1)K-1}_{K-1}}={b^{(2q+1)K}_{K-1}}.
    # 
    # XO=true ==> add extra "folding" operations to the algebra.
    #
    local i, j, k, kk, z, x, K, aa, a, bb, b, bi, bj, blk, Bmp, B0, B1, BB, p0, p, pi, r, q, g, gp0, gens, Gens, numgens, N, n, 
          ...
          uacalcpath, uacalcfile, STORE_GSET, numalgebras, yetfound, f1, f2, s0, s, f, XO;
    ...
    if nargin < 4 then XO:=false; else XO:=args[4]; fi;
    if nargin < 5 then algebraname:=StructureDescription(G); else algebraname:=args[5]; fi;
    ...
    if XO then
        filename:=Concatenation(uacalcpath, "Overalgebra3-XO_", algebraname, "_");
    else
    ...
    ### Experimental folding operations: 
    ### f1 -- maps B0 onto BB[2*K] and BB[k] onto BB[K+k]
    ### f2 -- maps BB[K] onto BB[3*K] and BB[K+k] onto BB[2*K+k]
    f:=[];
    for i in [1..Q] do
    ...
    if XO then
        AppendTo(uacalcfile, "<desc>Overalgebra3 XO: tie-points=", Gens-1,", Q=", Q, ", plus folding ops.</desc>\n");
    else
        AppendTo(uacalcfile, "<desc>Overalgebra3: tie-points=", Gens-1,", Q=", Q, ".</desc>\n");
    fi;
    AppendTo(uacalcfile, "<cardinality>", sizeA, "</cardinality>\n");
    AppendTo(uacalcfile, "<operations>\n");
    
    
    ### We don't need to add the p0 and p[k]'s, once we have qij's (below).
    ## p0  N.B. we call this functions e_0 in the paper. ## 
    # writeUACalcOp(uacalcfile, p0, "e0");
    # ## p[k]  N.B. we call these functions e_i in the paper. ## 
    # for k in [1..Length(p)] do
    #     opname:=Concatenation("e",String(k));
...
OveralgebraExample:=function(number)
    ...
    if number=1 then
        ...
        # Create an overalgebra III with Q=2 and tie-points T={0, 2} = {a[1], b[1]}, 
        # (so K=2).  Then there are (2Q+1)K=10 potatos:
        # B0, B[1], B[2], ..., B[(2*Q+1)K] = B[10] 
        # and (2Q+1)+1 = 6 ears:
        # B0, B[K], B[2K], B[3K], B[4K], B[5K]
        Overalgebra3([G, [[0,2]], 2]);
        # Since there are 6 ears, three even and three odd,
        # the interval [\beta^*, \hatbeta] will be Eq(3)xEq(3)
        
        Overalgebra3([G, [[0,2]], 2, true]);
        # This adds the folding operations with the result that 
        # [\beta^*, \hatbeta] is 3 x 3
