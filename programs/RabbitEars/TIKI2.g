...
TIKI2:=function(args)
    ...
    local B0, BB, p0, s, s1, s2, q0, q1, gn, gensB, gensA, nargin, G, pathname, filename, outfile, N, m, opcount, g, k, z, x, pos, pos1, pos2, 
          ...
          uacalcpath, uacalcfile, STORE_GSET, e, numalgebras, znotfound;
    ...
    for g in [1..ngens] do
        for k in [1..genlen[g]] do
            pos1:=Position(axes,Axes[g][k]);
            for z in [1..sizeA] do
                znotfound:=true;
                for j in [1..genlen[g]] do
                    pos2:=Position(axes,Axes[g][j]);
                    if z in BB[pos2] then
                        q0[pos1][z]:=(z^p0[pos2])^p0[pos1];  # BB[pos2] --> B0 --> BB[pos1]
                        znotfound:=false;
                        break;
                    fi;
                od;
                if znotfound then           
                    q0[pos1][z]:= Axes[g][k];   # otherwise collapse to point B0 \cap BB[pos1]
                fi;
            od;
        od;
    od;
    ...
    #for m in [1..M] do
    for m in [1..naxes] do
        opname:=Concatenation("q0",String(m));
        writeUACalcOp(uacalcfile, q0[m], opname);
    od;
